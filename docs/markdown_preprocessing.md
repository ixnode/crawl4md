...
The profile provides defaults for `crawl` and `preprocessing.markdown`. Project-level `crawl` and `preprocessing` values override profile defaults, so you can adjust individual options without copying the whole profile. Built-in profiles are loaded from `profiles/*.yml` (for example `profiles/wikipedia.yml`). HTML normalization is configured separately via `normalization` in project config.
